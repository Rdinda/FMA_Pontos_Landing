...
FROM php:8.4-fpm-alpine AS runtime-app
...
RUN sed -i 's~^;*clear_env\\s*=\\s*yes~clear_env = no~' /usr/local/etc/php-fpm.d/www.conf \
    && sed -i 's~^listen\\s*=.*~listen = 0.0.0.0:9000~' /usr/local/etc/php-fpm.d/www.conf
...
FROM alpine:3.19 AS runtime-nginx
...
FROM php:8.4-cli-alpine AS runtime-supervisor
...
COPY docker/entrypoint-supervisor.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh
